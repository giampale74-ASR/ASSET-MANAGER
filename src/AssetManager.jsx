...
export default function AssetManager() {
  ...
  const [hwColWidths, setHwColWidths] = useState({});
  const hwResizingCol   = useRef(null);
  const hwResizingStartX= useRef(0);
  const hwResizingStartW= useRef(0);
  const hwTableRef      = useRef(null);
    ...
    <div style={{fontFamily:"'Inter','JetBrains Mono',sans-serif",background:T.bg,minHeight:"100vh",color:T.text,transition:"background .2s,color .2s"}}>
      ...
      <div style={{borderBottom:`1px solid ${T.border}`,padding:"12px 24px",display:"flex",alignItems:"center",gap:12,background:T.surface,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontWeight:700,fontSize:17,letterSpacing:"-0.02em",color:T.text}}><span style={{color:T.accent,fontWeight:800}}>Hili</span>{" "}Asset Manager</div>
      ...
      <div style={{padding:"24px 20px 48px",maxWidth:1800,margin:"0 auto"}}>
        ...
        {view==="table"&&<div className="ai">
          ...
          {(()=>{
            ...
            return <div style={{background:T.surface,borderRadius:14,border:`2px solid ${T.border2}`,overflow:"hidden",boxShadow:`0 2px 16px rgba(0,0,0,0.18)`}}>
              <div style={{overflowX:"auto"}} ref={tableRef}>
                <table style={{tableLayout:"fixed",borderCollapse:"collapse",fontSize:13,minWidth:"100%"}}>
                  ...
                  <thead><tr style={{background:T.surface2,borderBottom:`2px solid ${T.border2}`}}>
              ...
              <div style={{padding:"10px 16px",borderTop:`2px solid ${T.border2}`,fontSize:12,color:T.textSub,display:"flex",gap:16,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {view==="detail"&&detailAsset&&<div className="ai">
          ...
          {(()=>{
            ...
            return <div style={{maxWidth:900}}><div style={{fontSize:12,color:T.textSub,fontWeight:600,marginBottom:12}}>🕓 Storico di questo asset</div>
              <div style={{background:T.surface,border:`2px solid ${T.border2}`,borderRadius:10,overflow:"hidden"}}>
        ...
        {view==="hardware"&&<div className="ai">
          {hwView==="table"?(
            <div>
              ...
              <div style={{background:T.surface,borderRadius:14,border:`2px solid ${T.border2}`,overflow:"hidden",boxShadow:`0 2px 16px rgba(0,0,0,0.18)`}}>
                {(()=>{
                  const hwCols = [["tipo","Tipo"],["marca","Marca"],["modello","Modello"],["seriale","Seriale"],["stato","Stato"],["assegnatoA","Assegnato a"],["note","Note"]];
                  const startHwResize=(e,key)=>{
                    e.preventDefault();e.stopPropagation();
                    const thEl=e.currentTarget.parentElement;
                    hwResizingCol.current=key;hwResizingStartX.current=e.clientX;hwResizingStartW.current=hwColWidths[key]||thEl.offsetWidth;
                    document.body.style.cursor="col-resize";document.body.style.userSelect="none";
                    const onMove=(ev)=>{const delta=ev.clientX-hwResizingStartX.current;const nw=Math.max(60,hwResizingStartW.current+delta);setHwColWidths(prev=>({...prev,[hwResizingCol.current]:nw}));};
                    const onUp=()=>{hwResizingCol.current=null;document.body.style.cursor="";document.body.style.userSelect="";window.removeEventListener("mousemove",onMove);window.removeEventListener("mouseup",onUp);};
                    window.addEventListener("mousemove",onMove);window.addEventListener("mouseup",onUp);
                  };
                  return <div style={{overflowX:"auto"}} ref={hwTableRef}>
                    <table style={{tableLayout:"fixed",borderCollapse:"collapse",fontSize:13,minWidth:"100%"}}>
                      <colgroup>{hwCols.map(([f])=><col key={f} style={{width:hwColWidths[f]?hwColWidths[f]+"px":undefined}}/>)}<col style={{width:"100px"}}/></colgroup>
                      <thead><tr style={{background:T.surface2,borderBottom:`2px solid ${T.border2}`}}>
                        {hwCols.map(([f,l])=>(
                          <th key={f} className="th" style={{padding:"10px 14px",textAlign:"left",color:T.textSub,fontWeight:500,fontSize:11,textTransform:"uppercase",letterSpacing:"0.07em",position:"relative",whiteSpace:"nowrap",width:hwColWidths[f]||undefined}}>
                            <span onClick={()=>hwSortBy(f)} style={{display:"inline-flex",alignItems:"center",gap:2,cursor:"pointer"}}>{l}<span style={{opacity:hwSortField===f?1:0.3,marginLeft:4,fontSize:10}}>{hwSortField===f&&hwSortDir==="desc"?"▼":"▲"}</span></span>
                            <span onMouseDown={e=>startHwResize(e,f)} style={{position:"absolute",top:0,right:0,width:6,height:"100%",cursor:"col-resize",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2}} onMouseEnter={e=>e.currentTarget.style.background="#1f6feb55"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><span style={{width:2,height:"60%",background:"currentColor",borderRadius:1,opacity:0.4,pointerEvents:"none"}}/></span>
                          </th>
                        ))}
                        <th style={{padding:"10px 14px",color:T.textSub,fontSize:11,textTransform:"uppercase"}}>Azioni</th>
                      </tr></thead>
                      <tbody>
                        ...
                        :hwFiltered.map((h,i)=>{
                          ...
                          return <tr key={h.id} className="rh" style={{borderBottom:`1px solid ${T.border2}`,background:i%2===0?"transparent":T.surface2}}>
                            <td style={{padding:"11px 14px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}><span style={{fontWeight:500,color:T.text}}>{ti} {h.tipo}</span></td>
                            <td style={{padding:"11px 14px",color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.marca||"—"}</td>
                            <td style={{padding:"11px 14px",color:T.textSub,fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.modello||"—"}</td>
                            <td style={{padding:"11px 14px",fontWeight:500,color:T.link,fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.seriale||"—"}</td>
                            <td style={{padding:"11px 14px"}}><span className="badge" style={{background:sc+"22",color:sc,border:`1px solid ${sc}44`}}>{h.stato}</span></td>
                            <td style={{padding:"11px 14px",color:T.textSub,fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.assegnatoA||"—"}</td>
                            <td style={{padding:"11px 14px",color:T.textMuted,fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.note||"—"}</td>
                            <td style={{padding:"11px 14px"}}><div style={{display:"flex",gap:6}}>
                ...
                <div style={{padding:"10px 16px",borderTop:`2px solid ${T.border2}`,fontSize:12,color:T.textSub,display:"flex",alignItems:"center",gap:16}}>
                  <span>{hwFiltered.length} di {hardware.length} record</span>
                  {Object.keys(hwColWidths).length>0&&<button className="btn btn-ghost" style={{padding:"2px 8px",fontSize:11,marginLeft:"auto"}} onClick={()=>setHwColWidths({})}>↺ Reset larghezze</button>}
                </div>
              </div>
            </div>
          ):(
        ...
        {view==="history"&&<div className="ai">
          ...
          <div style={{background:T.surface,border:`2px solid ${T.border2}`,borderRadius:10,overflow:"hidden"}}>
            ...
            :filteredH.map((h,i)=><div key={h.id} className="rh" style={{padding:"14px 20px",borderBottom:i<filteredH.length-1?`1px solid ${T.border2}`:"none",display:"flex",gap:16,alignItems:"flex-start"}}>
        ...
        {view==="import"&&<div className="ai" style={{maxWidth:820}}>
          ...
          {!importData?(
            ...
            <div>
              ...
              <div style={{background:T.surface,border:`2px solid ${T.border2}`,borderRadius:10,overflow:"hidden",marginBottom:16}}>
                <div style={{padding:"10px 16px",borderBottom:`2px solid ${T.border2}`,fontSize:12}}><span style={{color:"#22c55e",fontWeight:600}}>✓ {importData.records.length} record pronti</span></div>
