  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setAssets(aR.data || []);
        setHardware(hR.data || []);
        // Normalizza i nomi colonna dal DB (potrebbero essere snake_case o camelCase)
        setHistory((hiR.data || []).map(h => ({
          ...h,
          assetId:     h.assetId     ?? h.asset_id     ?? null,
          assetSerial: h.assetSerial ?? h.asset_serial ?? null,
          assetNome:   h.assetNome   ?? h.asset_nome   ?? null,
        })));
...
const payload = {
  ...
  const addHistory = async (action, asset, changes) => {
    const entry = {
      ...
      assetSerial:asset?.serialePC   || null,
      assetNome:  asset?.nominativo  || null,
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      if (hd) setHistory(prev=>[{...hd,assetId:hd.assetId??hd.asset_id??null,assetSerial:hd.assetSerial??hd.asset_serial??null,assetNome:hd.assetNome??hd.asset_nome??null},...prev]);
