  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setAssets(aR.data || []);
        setHardware(hR.data || []);
        setHistory((hiR.data || []).map(h => ({
          ...h,
          ts:           Number(h.ts),
          asset_id:     h.asset_id     ?? null,
          asset_serial: h.asset_serial ?? null,
          asset_nome:   h.asset_nome   ?? null,
...
const payload = {
  ...
  const addHistory = async (action, asset, changes) => {
    ...
    try {
      ...
      if (error) { showToast("Errore storico: " + error.message, "error"); console.error("history insert error:", error); return; }
      if (data) setHistory(prev => [data, ...prev]);
    } catch (err) { showToast("Errore storico: " + err.message, "error"); console.error("history catch:", err); }
  };
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      const { data:hd, error:hiErr } = await supabase.from("history").insert(hi).select().single();
      if (hiErr) { showToast("Errore storico import: " + hiErr.message, "error"); console.error("history import error:", hiErr); }
      if (hd) setHistory(prev=>[{...hd, ts:Number(hd.ts)},...prev]);
        ...
        {view==="history"&&<div className="ai">
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,flexWrap:"wrap"}}>
            ...
            <button className="btn btn-ghost" style={{fontSize:12,padding:"6px 12px"}} onClick={async()=>{
              ...
              if(error){showToast("Errore ricarica storico: "+error.message,"error");return;}
              setHistory((data||[]).map(h=>({...h,ts:Number(h.ts)})));
              showToast("Storico aggiornato");
            }}>↺ Aggiorna</button>
