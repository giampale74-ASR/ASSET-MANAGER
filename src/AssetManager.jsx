...
const fmtTs  = (ts) => { try { if(!ts) return "—"; const d=new Date(ts); return isNaN(d)?String(ts):d.toLocaleString("it-IT"); } catch{ return "—"; } };
const parseChanges = (changes) => { if(!changes) return null; if(Array.isArray(changes)) return changes; try { const p=typeof changes==="string"?JSON.parse(changes):changes; return Array.isArray(p)?p:null; } catch{ return null; } };
