    ...
    try {
      if (hwEditId) {
        ...
        if (data.tipo==="PC"&&data.assegnatoA) {
          const sync={serialePC:data.seriale,modelloPC:`${data.marca} ${data.modello}`.trim(),dataAcquisto:data.dataAcquisto||"",dataConsegna:data.dataConsegna||""};
          await supabase.from("assets").update(sync).eq("nominativo",data.assegnatoA);
          setAssets(prev=>prev.map(a=>a.nominativo===data.assegnatoA?{...a,...sync}:a));
        }
        const old = hardware.find(h=>h.id===hwEditId);
        if (old?.tipo==="PC"&&old.assegnatoA&&old.assegnatoA!==data.assegnatoA) {
          await supabase.from("assets").update({serialePC:"",modelloPC:"",dataAcquisto:"",dataConsegna:""}).eq("nominativo",old.assegnatoA).eq("serialePC",old.seriale);
          setAssets(prev=>prev.map(a=>a.nominativo===old.assegnatoA&&a.serialePC===old.seriale?{...a,serialePC:"",modelloPC:"",dataAcquisto:"",dataConsegna:""}:a));
        }
