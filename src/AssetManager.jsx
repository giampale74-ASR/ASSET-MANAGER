...
const api = {
  ...
  post: (url, body) => fetch(url, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(body) }).then(r => { if(!r.ok) throw new Error(r.statusText); return r.json(); }),
  put: (url, body) => fetch(url, { method:"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify(body) }).then(r => { if(!r.ok) throw new Error(r.statusText); return r.json(); }),
  del: (url, body) => fetch(url, { method:"DELETE", headers:{"Content-Type":"application/json"}, body:body?JSON.stringify(body):undefined }).then(r => { if(!r.ok) throw new Error(r.statusText); return r.json(); }),
};
...
const stati = ["Attivo","Onboarding","Cessato"];
...
const ALL_COLUMNS = [
  ...
  { key:"numeroCellulare", label:"Tel",      always:false },
  { key:"sim",             label:"SIM",            always:false },
  { key:"accountMicrosoft",label:"Account M365",   always:false },
  { key:"note",            label:"Note",           always:false },
  { key:"stato",           label:"Stato Utente",          always:false },
  { key:"hardware",        label:"Hardware",       always:false },
];

const THEMES = {
  black: { bg:"#1a1c26",surface:"#23263a",surface2:"#1e2133",border:"#3a4060",border2:"#5a6490",text:"#eef1fa",textSub:"#a8b4d0",textMuted:"#6a7899",hover:"#2a2f46",inputBg:"#1e2133",selectBg:"#23263a",link:"#7eb8ff",tabOn:"#2a2f46",accent:"#5b7cfa",scrollTr:"#23263a",scrollTh:"#5a6490" },
  dark:  { bg:"#1e2a3f",surface:"#263350",surface2:"#1e2a3f",border:"#3f5580",border2:"#5a7ab0",text:"#e4eaf8",textSub:"#9ab0d8",textMuted:"#607098",hover:"#2d3e5e",inputBg:"#1e2a3f",selectBg:"#263350",link:"#89b8ff",tabOn:"#2d3e5e",accent:"#5b7cfa",scrollTr:"#263350",scrollTh:"#5a7ab0" },
  light: { bg:"#f0f4fa",surface:"#ffffff",surface2:"#e8edf8",border:"#b0c0d8",border2:"#6b85b8",text:"#0d1117",textSub:"#253249",textMuted:"#5a6a88",hover:"#dde5f5",inputBg:"#ffffff",selectBg:"#ffffff",link:"#1a56db",tabOn:"#d0dcf4",accent:"#3b5bdb",scrollTr:"#e8edf8",scrollTh:"#6b85b8" },
};

const statoColor = (s) => ({"Attivo":"#22c55e","Onboarding":"#f59e0b","Cessato":"#ef4444"}[s]||"#888");
const fmtDate = (d) => d ? new Date(d).toLocaleDateString("it-IT") : "—";
const fmtTs = (ts) => { try { if(!ts) return "—"; const d=new Date(ts); return isNaN(d.getTime())?"—":d.toLocaleString("it-IT"); } catch{ return "—"; } };
const safeChanges = (ch) => { if(!ch) return null; if(Array.isArray(ch)) return ch; try { const p=typeof ch==="string"?JSON.parse(ch):ch; return Array.isArray(p)?p:null; } catch{ return null; } };
...
export default function AssetManager() {
  ...
  const [hwForm, setHwForm]     = useState({tipo:"",modello:"",seriale:"",stato:"In uso",assegnatoA:"",dataAcquisto:"",dataConsegna:"",note:""});
  ...
  const [visibleCols, setVisibleCols]   = useState(["nominativo","reparto","serialePC","modelloPC","numeroCellulare","dataConsegna","stato","hardware"]);
  ...
  const emptyHwForm = {tipo:"",modello:"",seriale:"",stato:"In uso",assegnatoA:"",dataAcquisto:"",dataConsegna:"",note:""};
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [assets, hardware, history, reparti, checks, labels] = await Promise.all([
          api.get("/api/assets"),
          api.get("/api/hardware"),
          api.get("/api/history"),
          api.get("/api/reparti"),
          api.get("/api/checks"),
          api.get("/api/check-labels"),
        ]);
        setAssets(assets || []);
        setHardware(hardware || []);
        setHistory(history || []);
        setReparti((reparti||[]).map(r=>r.nome).filter(Boolean).length > 0 ? (reparti||[]).map(r=>r.nome) : defaultRepartiList);
        const chMap = {};
        (checks||[]).forEach(c => { chMap[c.key] = !!c.value; });
        setChecks(chMap);
        const lbs = (labels||[]).sort((a,b)=>a.idx-b.idx);
        if (lbs.length === 4) setCheckLabels(lbs.map(l=>l.label));
      } catch (err) {
        setDbError(err.message || "Errore connessione al server");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const filtered = useMemo(() => {
    let d = [...assets];
    if (search) { const q=search.toLowerCase(); d=d.filter(a=>["nominativo","serialePC","numeroCellulare","accountMicrosoft","email","modelloPC"].some(k=>(a[k]||"").toLowerCase().includes(q))); }
...
const payload = {
  ...
  stato: form.stato
    ...
    try {
      if (editId) {
        ...
        const data = await api.put(`/api/assets/${editId}`, payload);
        setAssets(prev=>prev.map(a=>a.id===editId?data:a));
        if (data.modelloPC) await ensureHardwareFromAsset(data.nominativo, data.modelloPC, data.serialePC);
  ...
  const doDelete = async () => {
    ...
    try {
      await api.del(`/api/assets/${deleteConfirm}`);
  ...
  const addHistory = async (action, asset, changes) => {
    ...
    try {
      ...
      await reloadHistory();
    } catch (err) { showToast("Errore storico: " + err.message, "error"); }
  };

  // ── sync asset.modelloPC → hardware.modello ──
  const syncHardwareFromAsset = async (nominativo, modelloPC) => {
    if (!nominativo || !modelloPC) return;
    const hw = hardware.find(h => h.tipo === "PC" && h.assegnatoA && h.assegnatoA.toLowerCase() === nominativo.toLowerCase());
    if (!hw) return;
    await api.put(`/api/hardware/${hw.id}`, { ...hw, modello: modelloPC });
    setHardware(prev => prev.map(h => h.id === hw.id ? { ...h, modello: modelloPC } : h));
  };

  // ── create hardware PC record if none exists for this asset ──
  const ensureHardwareFromAsset = async (nominativo, modelloPC, serialePC) => {
    if (!nominativo || !modelloPC) return;
    const hw = hardware.find(h => h.tipo === "PC" && h.assegnatoA && h.assegnatoA.toLowerCase() === nominativo.toLowerCase());
    if (hw) {
      await api.put(`/api/hardware/${hw.id}`, { ...hw, modello: modelloPC });
      ...
      const newHw = { tipo: "PC", modello: modelloPC, seriale: serialePC || "", stato: "In uso", assegnatoA: nominativo, note: "" };
      const data = await api.post("/api/hardware", newHw);
      if (data) setHardware(prev => [...prev, data]);
    }
  };
  ...
  const addReparto = async (nome) => {
    ...
    try {
      await api.post("/api/reparti", { nome });
      setReparti(prev=>[...prev,nome]);
    } catch (err) { showToast("Errore: "+err.message,"error"); }
  };
  const removeReparto = async (nome) => {
    try {
      await api.del("/api/reparti", { nome });
  ...
  const toggleCheck = async (assetId, idx) => {
    ...
    try {
      await api.post("/api/checks/upsert", { key, value: newVal });
  ...
  const saveCheckLabel = async (idx, label) => {
    ...
    try { await api.post("/api/check-labels/upsert", { idx, label }); } catch {}
  };
  ...
  const saveHwForm = async () => {
    ...
    try {
      if (hwEditId) {
        const data = await api.put(`/api/hardware/${hwEditId}`, payload);
        setHardware(prev=>prev.map(h=>h.id===hwEditId?data:h));
        if (data.assegnatoA) {
          const syncAsset = {
            ...(data.tipo==="PC" ? {serialePC:data.seriale, modelloPC:data.modello} : {}),
            dataAcquisto: data.dataAcquisto || "",
            dataConsegna: data.dataConsegna || "",
          };
          await api.put(`/api/hardware/${hwEditId}/sync-asset`, { ...data });
          setAssets(prev=>prev.map(a=>a.nominativo===data.assegnatoA?{...a,...syncAsset}:a));
        }
        const old = hardware.find(h=>h.id===hwEditId);
        if (old?.assegnatoA&&old.assegnatoA!==data.assegnatoA) {
          const clearAsset = {
            ...(old.tipo==="PC" ? {serialePC:"", modelloPC:""} : {}),
            dataAcquisto: "",
            dataConsegna: "",
          };
          setAssets(prev=>prev.map(a=>a.nominativo===old.assegnatoA?{...a,...clearAsset}:a));
        }
        await reloadHardware();
        showToast("Hardware aggiornato!");
      } else {
        const data = await api.post("/api/hardware", payload);
        if (data.assegnatoA) {
          const syncAsset = {
            ...(data.tipo==="PC" ? {serialePC:data.seriale, modelloPC:data.modello} : {}),
            dataAcquisto: data.dataAcquisto || "",
          ...
          await api.put(`/api/hardware/${data.id}/sync-asset`, { ...data });
          setAssets(prev=>prev.map(a=>a.nominativo===data.assegnatoA?{...a,...syncAsset}:a));
        }
        await reloadHardware();
  ...
  const confirmImport = async () => {
    ...
    try {
      if (importMode==="sostituisci") {
        await api.del("/api/assets/all");
        setAssets([]);
      }
      const cleanVal = (v) => v === "" ? null : v;
      const toInsert = importData.records.map(r=>({nominativo:r.nominativo,email:cleanVal(r.email),reparto:cleanVal(r.reparto),serialePC:cleanVal(r.serialePC),modelloPC:cleanVal(r.modelloPC),dataAcquisto:cleanVal(r.dataAcquisto),dataConsegna:cleanVal(r.dataConsegna),numeroCellulare:cleanVal(r.numeroCellulare),accountMicrosoft:cleanVal(r.accountMicrosoft),note:cleanVal(r.note),stato:r.stato}));
      let inserted = 0, updated = 0;
      const currentAssets = importMode==="sostituisci" ? [] : assets;
      for (const rec of toInsert) {
        const existing = currentAssets.find(a => a.nominativo === rec.nominativo);
        if (existing) {
          await api.put(`/api/assets/${existing.id}`, rec);
          updated++;
        } else {
          await api.post("/api/assets", rec);
          inserted++;
        }
        if (rec.modelloPC) await ensureHardwareFromAsset(rec.nominativo, rec.modelloPC, rec.serialePC);
      }
      const reloaded = await api.get("/api/assets");
      setAssets(reloaded || []);
      await reloadHardware();
      const total = inserted + updated;
      await api.post("/api/history", {
        ts: new Date().toISOString(),
        action: importMode==="sostituisci" ? "IMPORTAZIONE_COMPLETA" : "IMPORTAZIONE",
        asset_id: null, asset_serial: null,
        asset_nome: `${total} asset`, changes: null
      });
      await reloadHistory();
      showToast(`${total} asset importati! (${updated} aggiornati, ${inserted} nuovi)`);
    ...
    <div style={{fontFamily:"'Inter',sans-serif",background:"#131929",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",color:"#dde5f5"}}>
      <div style={{textAlign:"center",maxWidth:500,padding:32}}>
        ...
        <div style={{fontSize:13,color:"#7f90b8",marginBottom:20,textAlign:"left",lineHeight:1.9}}>
          Verifica che il server Node.js sia in esecuzione e che la variabile d'ambiente sia configurata:<br/>
          <code style={{color:"#82aaff"}}>DB_PATH</code> → percorso cartella database (es. <code style={{color:"#82aaff"}}>/home/data</code>)<br/><br/>
          Controlla i log del Servizio App Azure per ulteriori dettagli.
        </div>
  ...
  return (
    <div style={{fontFamily:"'Inter','JetBrains Mono',sans-serif",background:T.bg,minHeight:"100vh",color:T.text,transition:"background .2s,color .2s"}}>
      ...
      <div style={{padding:"24px 32px 48px",maxWidth:"100%",margin:"0 auto"}}>
          ...
          {(()=>{
            ...
            const renderCell=(a,key,w)=>{
              ...
              switch(key){
                ...
                case "modelloPC": {
                  const hwPc=hardware.find(h=>h.tipo==="PC"&&h.assegnatoA&&h.assegnatoA.toLowerCase()===a.nominativo.toLowerCase());
                  return <td key={key} style={{...cs,fontSize:12}}>
                    {hwPc
                      ? <span onClick={()=>{setHwEditId(hwPc.id);setHwForm({...hwPc});setHwView("form");setView("hardware");}} style={{color:T.link,cursor:"pointer",textDecoration:"underline",textDecorationStyle:"dotted"}} title="Vai all'hardware">{hwPc.modello||"—"}</span>
                ...
                case "hardware":{
                  ...
                  return <td key={key} style={{...cs,maxWidth:w||200}}>{ass.length===0?<span style={{color:T.textMuted,fontSize:12}}>—</span>:<div style={{display:"flex",flexWrap:"wrap",gap:4}}>{ass.map(h=><span key={h.id} title={`${h.modello} · ${h.seriale}`} onClick={()=>{setHwEditId(h.id);setHwForm({...h});setHwView("form");setView("hardware");}} style={{display:"inline-flex",alignItems:"center",gap:3,background:T.surface2,border:`1px solid ${T.border}`,borderRadius:4,padding:"2px 6px",fontSize:11,color:T.link,whiteSpace:"nowrap",cursor:"pointer"}}>{ti[h.tipo]||"📦"} {h.tipo}</span>)}</div>}</td>;
                }
        ...
        {view==="form"&&<div className="ai">
          ...
          <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,padding:28,maxWidth:900}}>
            {[
              {icon:"👤",title:"Dati Dipendente",grid:"1fr 1fr",fields:[["nominativo","Nominativo *","text","Nome Cognome"],["email","Email aziendale","email","nome@azienda.it"],["reparto","Ruolo","reparto",null],["numeroCellulare","Tel","text","+39 3XX XXXXXXX"],["accountMicrosoft","Email personale","text","nome@gmail.com"]]},
            ...
            <div style={{marginBottom:28}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:16}}>
                <div className="fi"><label className="fl">Stato Utente</label><select className="fi2" value={form.stato} onChange={e=>setForm({...form,stato:e.target.value})}>{stati.map(s=><option key={s}>{s}</option>)}</select></div>
        ...
        {view==="detail"&&detailAsset&&<div className="ai">
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:16,maxWidth:900,marginBottom:24}}>
            {[{icon:"👤",title:"Dipendente",rows:[["Nominativo",detailAsset.nominativo],["Email",detailAsset.email],["Ruolo",detailAsset.reparto],["Tel",detailAsset.numeroCellulare],["Email personale",detailAsset.accountMicrosoft],["Note",detailAsset.note||"—"]]}].map(({icon,title,rows})=>(
          ...
          {(()=>{
            const ti={"PC":"🖥","Monitor":"🖥","Telefono":"📱","SIM":"📡","Cuffie":"🎧","Altro":"📦"};
            const sc={"In uso":"#22c55e","Disponibile":"#1f6feb","In manutenzione":"#f59e0b","Dismesso":"#ef4444"};
            const assignedHw=hardware.filter(h=>h.assegnatoA&&h.assegnatoA.toLowerCase()===detailAsset.nominativo.toLowerCase());
            if(!assignedHw.length) return null;
            return <div style={{maxWidth:900,marginBottom:24}}>
              <div style={{fontSize:12,color:T.textSub,fontWeight:600,marginBottom:12}}>🖥 Hardware assegnato</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12}}>
                {assignedHw.map(h=><div key={h.id} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:16}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                    <span style={{fontSize:20}}>{ti[h.tipo]||"📦"}</span>
                    <span style={{fontWeight:600,fontSize:13,color:T.text}}>{h.tipo}</span>
                    <span className="badge" style={{marginLeft:"auto",background:(sc[h.stato]||"#888")+"22",color:sc[h.stato]||"#888",border:`1px solid ${(sc[h.stato]||"#888")}44`,fontSize:10}}>{h.stato}</span>
                  </div>
                  {[["Modello",h.modello],["Seriale",h.seriale],["Acquisto",h.dataAcquisto?new Date(h.dataAcquisto).toLocaleDateString("it-IT"):null],["Consegna",h.dataConsegna?new Date(h.dataConsegna).toLocaleDateString("it-IT"):null],["Note",h.note]].map(([k,v])=>v?<div key={k} style={{marginBottom:6}}><span style={{fontSize:11,color:T.textMuted}}>{k}: </span><span style={{fontSize:12,color:T.textSub}}>{v}</span></div>:null)}
                </div>)}
              </div>
            </div>;
          })()}
            ...
            return <div style={{maxWidth:900}}><div style={{fontSize:12,color:T.textSub,fontWeight:600,marginBottom:12}}>🕓 Storico di questo asset</div>
              <div style={{background:T.surface,border:`3px solid ${T.border2}`,borderRadius:10,overflow:"hidden",boxShadow:`0 4px 20px rgba(0,0,0,0.25)`}}>
                {ah.map((h,i)=><div key={h.id} style={{padding:"12px 16px",borderBottom:i<ah.length-1?`1px solid ${T.border}`:"none"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:safeChanges(h.changes)?.length?8:0}}><ActionBadge action={h.action}/><span style={{fontSize:12,color:T.textSub}}>{fmtTs(h.ts)}</span></div>
                  {safeChanges(h.changes)?.map((c,ci)=><div key={ci} style={{marginTop:4,fontSize:12,color:T.textSub,paddingLeft:8}}><span style={{color:T.textMuted}}>{c.field}: </span><span style={{color:"#ef4444"}}>{String(c.from??"(vuoto)")}</span><span style={{color:T.textMuted}}> → </span><span style={{color:"#22c55e"}}>{String(c.to??"(vuoto)")}</span></div>)}
                </div>)}
              </div>
            </div>;
          })()}
          ...
          {hwView==="table"?(
            <div>
              <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
                <input className="fi2" placeholder="🔍  Cerca tipo, modello, seriale..." value={hwSearch} onChange={e=>setHwSearch(e.target.value)} style={{flex:1,minWidth:220}}/>
              ...
              <div style={{background:T.surface,borderRadius:14,border:`1px solid ${T.border}`,overflow:"hidden"}}>
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                    <thead><tr style={{background:T.surface2,borderBottom:`3px solid ${T.border2}`}}>
                      {[["tipo","Tipo"],["modello","Modello"],["seriale","Seriale"],["stato","Stato"],["assegnatoA","Assegnato a"],["dataAcquisto","Data Acquisto"],["dataConsegna","Data Consegna"],["note","Note"]].map(([f,l])=>(
                    ...
                    <tbody>
                      ...
                      :hwFiltered.map((h,i)=>{
                        ...
                        return <tr key={h.id} className="rh" style={{borderBottom:`1px solid ${T.border2}`,background:i%2===0?"transparent":T.surface2}}>
                          ...
                          <td style={{padding:"11px 14px",color:T.textSub,fontSize:12}}>{h.dataAcquisto?new Date(h.dataAcquisto).toLocaleDateString("it-IT"):"—"}</td>
                          <td style={{padding:"11px 14px",color:T.textSub,fontSize:12}}>{h.dataConsegna?new Date(h.dataConsegna).toLocaleDateString("it-IT"):"—"}</td>
            ...
            <div>
              ...
              <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,padding:28,maxWidth:700}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
                  {[["tipo","Tipo *","select"],["modello","Modello","text"],["seriale","Seriale *","text"],["stato","Stato","select2"],["assegnatoA","Assegnato a","select3"],["dataAcquisto","Data Acquisto","date"],["dataConsegna","Data Consegna","date"]].map(([key,label,type])=>(
                    <div key={key} className="fi"><label className="fl">{label}</label>
                      ...
                      :<input className="fi2" type={type==="date"?"date":"text"} placeholder={label} value={hwForm[key]||""} onChange={e=>setHwForm({...hwForm,[key]:e.target.value})}/>}
                    </div>
                  ))}
                </div>
        ...
        {view==="history"&&<div className="ai">
          ...
          <div style={{background:T.surface,border:`3px solid ${T.border2}`,borderRadius:10,overflow:"hidden",boxShadow:`0 4px 20px rgba(0,0,0,0.25)`}}>
            ...
            :filteredH.map((h,i)=><div key={h.id} className="rh" style={{padding:"14px 20px",borderBottom:i<filteredH.length-1?`1px solid ${T.border}`:"none",display:"flex",gap:16,alignItems:"flex-start"}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",gap:8,alignItems:"baseline",marginBottom:safeChanges(h.changes)?.length?6:0}}>
                ...
                {safeChanges(h.changes)?.map((c,ci)=><div key={ci} style={{fontSize:12,color:T.textSub,marginTop:3}}><span style={{color:T.textMuted}}>{c.field}: </span><span style={{color:"#ef4444"}}>{String(c.from??"(vuoto)")}</span><span style={{color:T.textMuted}}> → </span><span style={{color:"#22c55e"}}>{String(c.to??"(vuoto)")}</span></div>)}
              </div>
        ...
        {view==="import"&&<div className="ai" style={{maxWidth:820}}>
          ...
          <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:20,marginBottom:20}}>
            ...
            <div style={{fontSize:13,color:T.textSub,lineHeight:1.85}}>
              ...
              3. Stati validi: <span style={{color:T.text}}>Attivo · Onboarding · Cessato</span>.<br/>
