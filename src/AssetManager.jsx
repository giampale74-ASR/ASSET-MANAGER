...
const payload = {
  ...
  const addHistory = async (action, asset, changes) => {
    const entry = { ts:new Date().toISOString(),action,asset_id:asset?.id||null,asset_serial:asset?.serialePC||null,asset_nome:asset?.nominativo||null,changes:changes||null };
    try {
        ...
        {view==="history"&&<div className="ai">
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,flexWrap:"wrap"}}>
            ...
            <button className="btn btn-ghost" style={{fontSize:12,padding:"6px 12px"}} onClick={async()=>{
              ...
              setHistory(data||[]);
              showToast("Storico ricaricato ("+(data?.length||0)+" eventi)");
            }}>↺ Aggiorna</button>
