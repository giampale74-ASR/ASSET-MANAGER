...
const payload = {
  ...
  const reloadHistory = async () => {
    ...
    if (error) { showToast("Reload storico errore: " + error.message, "error"); return; }
    setHistory((data || []).map(h => ({ ...h, ts: Number(h.ts) })));
  };
  ...
  const addHistory = async (action, asset, changes) => {
    ...
    if (error) {
      showToast("❌ Storico errore: " + error.message + " | code: " + error.code, "error");
      return;
    }
    await reloadHistory();
  };
      ...
      <div style={{padding:"24px 20px 48px",maxWidth:1800,margin:"0 auto"}}>
        ...
        {view==="history"&&<div className="ai">
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,flexWrap:"wrap"}}>
            ...
            <div style={{flex:1}}/>
            <button className="btn btn-ghost" style={{fontSize:12,padding:"6px 12px"}} onClick={async()=>{
              // Test insert diretto per diagnostica
              const testEntry = {ts:Date.now(),action:"TEST",asset_id:null,asset_serial:null,asset_nome:"test diagnostica",changes:null};
              const {error} = await supabase.from("history").insert(testEntry);
              if(error) showToast("❌ Insert fallito: "+error.message+" ["+error.code+"]","error");
              else { await reloadHistory(); showToast("✓ Test insert OK — storico ricaricato"); }
            }}>🧪 Test</button>
            <button className="btn btn-ghost" style={{fontSize:12,padding:"6px 12px"}} onClick={async()=>{ await reloadHistory(); showToast("Storico ricaricato"); }}>↺ Aggiorna</button>
