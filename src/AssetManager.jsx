...
export default function AssetManager() {
  ...
  const [hwForm, setHwForm]     = useState({tipo:"",marca:"",modello:"",seriale:"",stato:"In uso",assegnatoA:"",dataAcquisto:"",dataConsegna:"",note:""});
  ...
  const emptyHwForm = {tipo:"",marca:"",modello:"",seriale:"",stato:"In uso",assegnatoA:"",dataAcquisto:"",dataConsegna:"",note:""};
...
const payload = {
  ...
  const addHistory = async (action, asset, changes) => {
    const entry = { ts:Date.now(),action,asset_id:asset?.id||null,asset_serial:asset?.serialePC||null,asset_nome:asset?.nominativo||null,changes:changes||null };
    try {
  ...
  const saveHwForm = async () => {
    ...
    try {
      if (hwEditId) {
        ...
        if (data.assegnatoA) {
          const syncAsset = {
            ...(data.tipo==="PC" ? {serialePC:data.seriale, modelloPC:`${data.marca} ${data.modello}`.trim()} : {}),
            dataAcquisto: data.dataAcquisto || "",
            dataConsegna: data.dataConsegna || "",
          };
          await supabase.from("assets").update(syncAsset).eq("nominativo",data.assegnatoA);
        ...
        if (old?.assegnatoA&&old.assegnatoA!==data.assegnatoA) {
          ...
          await supabase.from("assets").update(clearAsset).eq("nominativo",old.assegnatoA);
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      const hi = {ts:Date.now(),action:importMode==="sostituisci"?"IMPORTAZIONE_COMPLETA":"IMPORTAZIONE",asset_id:null,asset_serial:null,asset_nome:`${data.length} asset`,changes:null};
              ...
              <div style={{background:T.surface,borderRadius:14,border:`1px solid ${T.border}`,overflow:"hidden"}}>
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                    <thead><tr style={{background:T.surface2,borderBottom:`3px solid ${T.border2}`}}>
                      {[["tipo","Tipo"],["marca","Marca"],["modello","Modello"],["seriale","Seriale"],["stato","Stato"],["assegnatoA","Assegnato a"],["dataAcquisto","Data Acquisto"],["dataConsegna","Data Consegna"],["note","Note"]].map(([f,l])=>(
            ...
            <div>
              ...
              <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,padding:28,maxWidth:700}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
                  {[["tipo","Tipo *","select"],["marca","Marca","text"],["modello","Modello","text"],["seriale","Seriale *","text"],["stato","Stato","select2"],["assegnatoA","Assegnato a","select3"],["dataAcquisto","Data Acquisto","date"],["dataConsegna","Data Consegna","date"]].map(([key,label,type])=>(
