    ...
    try {
      if (hwEditId) {
        ...
        if (data.tipo==="PC"&&data.assegnatoA) {
          const syncFields={serialePC:data.seriale,modelloPC:`${data.marca} ${data.modello}`.trim(),dataAcquisto:data.dataAcquisto||"",dataConsegna:data.dataConsegna||""};
          await supabase.from("assets").update(syncFields).eq("nominativo",data.assegnatoA);
          setAssets(prev=>prev.map(a=>a.nominativo===data.assegnatoA?{...a,...syncFields}:a));
        }
