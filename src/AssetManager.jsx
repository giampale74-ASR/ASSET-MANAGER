            ...
            return <div style={{background:T.surface,borderRadius:14,border:`2px solid ${T.border2}`,overflow:"hidden",boxShadow:`0 2px 16px rgba(0,0,0,0.18)`}}>
              <div style={{overflowX:"auto"}} ref={tableRef}>
                <table style={{tableLayout:"fixed",borderCollapse:"collapse",fontSize:13,minWidth:"100%"}}>
                  <colgroup>{activeCols.map(col=><col key={col.key} style={{width:colWidths[col.key]?colWidths[col.key]+"px":undefined}}/>)}<col style={{width:colWidths["checks"]?colWidths["checks"]+"px":"220px"}}/><col style={{width:"120px"}}/></colgroup>
                  <thead><tr style={{background:T.surface2,borderBottom:`2px solid ${T.border2}`}}>
                    ...
                    <th style={{padding:"10px 14px",color:T.textSub,fontSize:11,textTransform:"uppercase",whiteSpace:"nowrap",width:colWidths["checks"]||220,position:"relative"}}>
                      ...
                      <span onMouseDown={e=>startResize(e,"checks")} style={{position:"absolute",top:0,right:0,width:6,height:"100%",cursor:"col-resize",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2}} onMouseEnter={e=>e.currentTarget.style.background="#1f6feb55"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><span style={{width:2,height:"60%",background:"currentColor",borderRadius:1,opacity:0.4,pointerEvents:"none"}}/></span>
                    </th>
                  ...
                  <tbody>
                    ...
                    :filtered.map((a,i)=><tr key={a.id} className="rh" style={{borderBottom:`1px solid ${T.border2}`,background:i%2===0?"transparent":T.surface2}}>
                      ...
                      <td style={{padding:"11px 14px",width:colWidths["checks"]||220,overflow:"hidden"}}><div style={{display:"flex",gap:8,alignItems:"center"}}>{checkLabels.map((_,idx)=><div key={idx} style={{display:"flex",flexDirection:"column",alignItems:"center",minWidth:46}}><div onClick={()=>toggleCheck(a.id,idx)} style={{width:20,height:20,borderRadius:5,cursor:"pointer",border:`2px solid ${getCheck(a.id,idx)?"#22c55e":T.border2}`,background:getCheck(a.id,idx)?"#22c55e":"transparent",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .15s"}}>{getCheck(a.id,idx)&&<span style={{color:"#fff",fontSize:12,fontWeight:700}}>✓</span>}</div></div>)}</div></td>
