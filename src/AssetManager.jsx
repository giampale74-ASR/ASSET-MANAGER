...
const payload = {
  ...
  const reloadHistory = async () => {
    const { data } = await supabase.from("history").select("*").order("ts", { ascending: false });
  ...
  const addHistory = async (action, asset, changes) => {
    ...
    if (error) { showToast("Errore storico: " + error.message, "error"); return; }
    await reloadHistory();
  };
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      const { error:hiErr } = await supabase.from("history").insert(hi);
      if (hiErr) showToast("Errore storico import: " + hiErr.message, "error");
